...
for epoch in tqdm(range(epochs)):
    ...
    for i, sample_batched in enumerate((train_loader)):
        ...
        label_s = torch.tensor(extend_label(graph[0, :3, :])).unsqueeze(0).cuda()
        label_g = torch.tensor(extend_label(graph[1, :3, :])).unsqueeze(0).cuda()
        matrix_s = torch.tensor(extend_matrix(graph[0, 3:, :])).unsqueeze(0).cuda()
