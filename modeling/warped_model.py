...
class LitLayoutParsing(LightningModule):
    ...
    def forward(self, x:tuple)-> None:
        input_ids, bbox, attention_mask, token_type_ids = x
        return self.model(input_ids=input_ids, bbox=bbox, attention_mask=attention_mask, token_type_ids=token_type_ids)
    ...
    def training_step(self, batch, batch_idx) -> None:
        ...
        s0,s1 = S[:,:,:3,:],S[:,:,3:,:]
        graph = np.array(batch['label'])
        #group part
        G = self.rel_g(self.dropout(reduce.unsqueeze(0)))
        g0,g1 = G[:,:,:3,:],G[:,:,3:,:]
        #### GROUND TRUTH
        label_s = torch.tensor(self.extend_label(graph[0, :3, :])).unsqueeze(0).cuda()
        label_g = torch.tensor(self.extend_label(graph[1, :3, :])).unsqueeze(0).cuda()
        matrix_s = torch.tensor(self.extend_matrix(graph[0, 3:, :])).unsqueeze(0).cuda()
        matrix_g = torch.tensor(self.extend_matrix(graph[1, 3:, :])).unsqueeze(0).cuda()
